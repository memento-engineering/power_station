...
- Changed (BREAKING, `FilingContract.evaluate`, `FilingRequirement`): filing now checks that a bead's text is VIABLE, not merely PRESENT. Six requirements land after the existing four, in enum order — `validation_plan_syntax`, `validation_plan_portability`, `repo_relative_paths`, `bead_references`, `release_versions`, `decision_references` — so `FilingReport.toJson` emits ten rows in the unchanged `{requirement, passed, detail}` shape and `passed` is true only when all ten pass. `evaluate` takes a required named `FilingEvidence`: the contract stays synchronous and pure, and the two non-executing shell parses, the all-status bead-id catalog and the recorded-decision union arrive through the new read-only `FilingEvidenceSource` (default `SystemFilingEvidenceSource`), which `FilingService` consults exactly once per inspection. Migration: a direct `evaluate` caller passes `evidence:`; `const FilingEvidence.unconsulted()` is correct for a bead that cites no id, no decision and carries no plan. `mountEligibilityFindings`, the filing JSON's top-level keys, the approval stamp keys and the approval-revision basis are all unchanged, so an existing receipt stays live.
- Added: `ValidationPlanProbe` / `SystemValidationPlanProbe` — the shared, non-executing `<shell> -n -c '( <plan> )'` seam. `SpecifyCapability` takes it by constructor injection and no longer spawns `sh` itself, so the stage that AUTHORS a plan and the gate that FILES it cannot disagree about whether it parses; Specify's fresh bd read, first-diagnostic wording and `invalidResult` supervision route are unchanged.
- Added: `lib/src/filing/filing_text.dart` — the pure bead-text scanners (`beadAnchors`, `absolutePathReferences`, `beadIdReferences`, `exactReleaseVersions`, `decisionReferences`, `validationPlanOffendingSlice`) shared by the discovery gather and the filing contract, so the anchor and decision-citation grammars are one implementation rather than two that would eventually disagree. `beadAnchors` now also reads drive-letter Windows file anchors; an absolute directory carrying no known extension is still not a file anchor. The five-field input, first-appearance order, `kMaxAnchors` bound and return shape are unchanged.
- Added: `UnconsultedFilingEvidenceSource` — the source that looks nothing up, for the caller that reads a bead and its approval revision and never a requirement row. The mount-eligibility recheck binds it: the revision digests bead content, which the viability rows contribute nothing to, so collecting their evidence would buy two process spawns and a store read per bead on a resident station's tick and change no answer that path reads. It is not a quiet pass — every row it yields says UNCHECKED. `ApproveService`'s default evidence source now reads bead ids through its own injected `runnerFor`, so a seat that composes ONE bd chokepoint keeps it.
- Changed: a failing viability row NAMES the offending text and what to do about it (`rewrite as one parseable POSIX-shell command`, `use a repository-relative path`, `mint it before citing it or cite an existing attached-store id`, …), because a check that only says "invalid" moves the guessing rather than removing it. An unreadable store, an unwired or crashed decision index and an unspawnable shell refuse by SOURCE and say so; none of them ever reads as proof that a cited reference is absent.
