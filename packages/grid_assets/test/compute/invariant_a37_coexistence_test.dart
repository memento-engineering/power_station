...
void main() {
  group('Invariant 5 — A37 / coexistence: a lease never writes a peer store '
    ...
    test('a real LeaseCapability over the loopback bus mutates the peer store ONLY '
        'via protocol dispatch; the lessee writes only its OWN store', () async {
    ...
    test(
      ...
      () async {
        ...
        addTearDown(server.close);
        final busClient = HttpStationClient(
          host: '127.0.0.1',
          port: server.port,
        );
        addTearDown(busClient.close);
