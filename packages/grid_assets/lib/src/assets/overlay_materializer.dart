...
/// - **The ROOT instruction file is COMPOSED, not owned.** [kAgentsRootRelativePath]
///   is a file the repository already has bytes in (its own prose, and `bd setup
///   codex`'s block), so this lib owns one marked BLOCK of it
///   ([MarkedBlockProvenance]) instead of the whole file: it writes the file when
///   absent, replaces only its own block on drift, appends its block beside a
///   complete bd block, and BLOCKS every other existing shape. No byte outside
///   its own boundary lines is ever rewritten.
...
///
/// [kAgentsRootRelativePath] is deliberately NOT here: it is a composed file this
/// lib owns one block of, never a head it may treat as its own territory.
const List<String> kOverlayTargetHeads = [kClaudeTargetHead, kAgentsTargetHead];

/// How `bd setup codex`'s own block OPENS in a repository's root instruction
/// file — a PREFIX, because bd stamps its generator and profile onto the rest of
/// the line.
///
/// Named here because it is the one foreign block this lib composes beside: a
/// root file carrying it is a file already understood to be tool-composed, which
/// is what makes appending to it safe.
const String kBeadsCodexBlockBeginPrefix = '<!-- BEGIN BEADS CODEX SETUP';

/// How `bd setup codex`'s own block CLOSES. See [kBeadsCodexBlockBeginPrefix].
const String kBeadsCodexBlockEnd = '<!-- END BEADS CODEX SETUP -->';
...
class OverlayFileBlocked extends OverlayFileOutcome {
  ...
  /// [detail] replaces the default reason for a refusal with its own cause (a
  /// composed file's marker shape), so the operator reads what to fix.
  const OverlayFileBlocked(
    super.relativePath, {
    this.symlink = false,
    this.detail,
  });
  ...
  /// The refusal's own cause, when it is not the plain missing-stamp one.
  final String? detail;
  ...
  String get reason => symlink
      ...
      : detail ??
            'exists WITHOUT a provenance header — hand-authored, never '
                'clobbered; delete it to let the vended file install';
}
...
class OverlayFileStale extends OverlayFileOutcome {
  /// Creates the stale outcome. [detail] replaces the default reason where the
  /// stale thing is one generated BLOCK inside a file the repository owns — an
  /// operator told "generated here" about their own root instruction file could
  /// delete far more than this lib ever wrote.
  const OverlayFileStale(super.relativePath, {this.detail});

  /// What exactly is stale, when it is not the whole file.
  final String? detail;
...
class OverlayMaterializer {
  ...
  ///
  /// [kAgentsRootRelativePath] alone is COMPOSED rather than owned — the same
  /// outcomes, decided over this lib's own marked BLOCK inside a file the
  /// repository owns the rest of (see [_materializeMarkedAgentsFile]).
  ///
  ...
  OverlayMaterializeReport materializeSync({
    ...
    for (final resolved in selected) {
      ...
      // The ROOT instruction file is COMPOSED: the repository owns bytes in it,
      // so this lib claims one marked block and never the whole file.
      if (p.normalize(relativePath) == kAgentsRootRelativePath) {
        files.add(
          _materializeMarkedAgentsFile(
            target: target,
  ...
  /// The outcome for the COMPOSED root instruction file: this lib owns the one
  /// marked block, and every byte outside it belongs to the repository.
  ///
  /// Absent ⇒ WRITTEN (the block alone). An existing file whose owned block this
  /// tooling can claim is UNCHANGED when its vended body already matches, and
  /// otherwise UPDATED by replacing that one span. An existing file with NO trace
  /// of our markers is composable only when it carries exactly one ordered
  /// `bd setup codex` block — the mark of a file already understood to be
  /// tool-composed — and then the block is APPENDED below it, altering nothing
  /// above. Every other shape is BLOCKED with its own cause and written not at
  /// all: neither marker, a partial or duplicated pair, or an owned block with no
  /// stamp on its first interior line (a hand-typed imitation).
  OverlayFileOutcome _materializeMarkedAgentsFile({
    ...
    if (!target.existsSync()) {
      if (!dryRun) {
        target.parent.createSync(recursive: true);
        target.writeAsStringSync(block);
      }
      return OverlayFileWritten(
        relativePath,
    ...
    if (owned == null) {
      if (marked.containsAnyMarker(existing)) {
        return OverlayFileBlocked(
          relativePath,
          detail:
              'carries grid_assets block markers this tooling cannot claim — '
              'there must be exactly ONE ordered '
              '`$kGeneratedBlockBegin` … `$kGeneratedBlockEnd` pair whose first '
              'interior line is its provenance stamp; repair or remove the '
              'markers to let the vended block install',
        );
      }
      if (!_hasComposableBeadsBlock(existing)) {
        return OverlayFileBlocked(
          relativePath,
          detail:
              'exists with instruction bytes this tooling never wrote and no '
              'complete `$kBeadsCodexBlockBeginPrefix …` / '
              '`$kBeadsCodexBlockEnd` block to compose beside — hand-authored '
              'instructions are never clobbered; run `bd setup codex`, or move '
              'the file aside, to let the vended block install',
        );
      }
    }
    final composed = marked.merge(existing, block: block);
    if (!dryRun) target.writeAsStringSync(composed);
    return OverlayFileUpdated(
      relativePath,
      sourceRoot: sourceRoot,
      contents: composed,
    );
  ...
  /// Whether [contents] carries exactly ONE ordered `bd setup codex` block.
  ///
  /// A partial or duplicated pair is NOT composable: this lib appends below a
  /// block it can see the whole of, and anything else is a file whose shape it
  /// does not understand well enough to add to.
  static bool _hasComposableBeadsBlock(String contents) {
    final lines = const LineSplitter().convert(contents);
    final begins = <int>[];
    final ends = <int>[];
    for (var i = 0; i < lines.length; i++) {
      final line = lines[i].trim();
      if (line.startsWith(kBeadsCodexBlockBeginPrefix)) begins.add(i);
      if (line == kBeadsCodexBlockEnd) ends.add(i);
    }
    return begins.length == 1 &&
        ends.length == 1 &&
        ends.single > begins.single;
  }
  ...
  List<OverlayFileStale> _staleOutcomes({
    ...
    // The composed root file is no head to walk — it is ONE path, and only an
    // unscoped install reaches it. Its BLOCK is what goes stale; the file around
    // it is the repository's and is never touched, here or anywhere.
    if (subtrees.isEmpty &&
        !selectedPaths.contains(kAgentsRootRelativePath) &&
        !_escapesTargetRoot(targetRoot, kAgentsRootRelativePath)) {
      final root = File(p.join(targetRoot, kAgentsRootRelativePath));
      if (root.existsSync() &&
          const MarkedBlockProvenance().bodyOf(root.readAsStringSync()) !=
              null) {
        stale.add(
          const OverlayFileStale(
            kAgentsRootRelativePath,
            detail:
                'carries a generated block the selected resolution no longer '
                'vends; the BLOCK is stale, the file around it is the '
                "repository's — left untouched",
          ),
        );
      }
    }
