...
/// up to the workspace `.dart_tool/package_config.json`). Since the repo split,
/// `grid_federation` lives in the sibling `the_grid` checkout and reaches this
/// suite via the machine-local path override — the package config is the one
/// authority on where it resolved. The `<pkg>.dart` barrel is the marker so a
/// `lib` candidate matches only its own package.
