...
- Changed: ADR-0004 D2's human-gate enumeration is NARROWED. It read "anything outward-facing beyond a branch push and PR", which swept in prerelease publishing — neither irreversible nor a human act, since pub excludes prereleases from stable caret ranges. It now names promoting a release instead, with an `updated-by` edge to the two org entries. No new decision minted.
