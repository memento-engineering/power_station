...
const _macosOnly = CapabilityFacts(sets: {kSystemOs: {'macos'}});
...
void main() {
  group('the claim flow (offline, two InMemoryBrokers)', () {
    test(
      'a fulfillable peer is confirmed; claimCapabilityFor resolves it; the '
      'retained ad is cleared',
      () async {
    ...
    test(
      'an UNFULFILLABLE peer (CapabilityFacts mismatch) never responds — '
      'the requirement stays pending',
      () async {
        final studio = InMemoryBroker(station: 'studio');
        final dashboard = InMemoryBroker(station: 'dashboard');
        addTearDown(studio.close);
    ...
    test(
      'deterministic arbitration: the FIRST responder wins; a later '
      'responder for the SAME claim is denied',
      () async {
        final studio = InMemoryBroker(station: 'studio');
    ...
    test(
      'a requirement that disappears (locally satisfied elsewhere) is '
      'RETRACTED — the retained ad clears',
      () async {
        final studio = InMemoryBroker(station: 'studio');
        addTearDown(studio.close);
        final owner = ClaimBroadcaster(broker: studio);
    ...
    test(
      'the SAME still-unclaimed requirement is never re-broadcast on a '
      'later scan (no retained churn)',
      () async {
        final studio = InMemoryBroker(station: 'studio');
        addTearDown(studio.close);
        final owner = ClaimBroadcaster(broker: studio);
