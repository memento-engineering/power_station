...
/// The complete block `bd setup codex` writes into that same file — the foreign
/// generated block this lib composes beside, byte for byte.
const String _beadsBlock =
...
void main() {
  ...
  group('OverlayMaterializer — the live grid_assets registry', () {
    test(
      ...
      () async {
        ...
        expect(body, contains('external:<project>:<capability>'));
        expect(body, contains('export:<target>'));
        expect(body, isNot(contains('type=link')));
        expect(body, isNot(contains('grid.link.')));
        expect(body, isNot(contains('StationJoinBridge._applyCrossLinks')));
        expect(body, isNot(contains('applyBlockGuard')));
  ...
  group('OverlayMaterializer — the COMPOSED root instruction file', () {
    TestAssetResolutionFixture rootFixture([
      String body = 'the org doctrine',
    ]) => TestAssetResolutionFixture(
      root: temp,
      assets: <GridAssetDefinition>[fixtureAgentsInstructions()],
      bodies: fixtureAgentsInstructionsBodies(body),
    );

    test('an ABSENT root file is written as the owned block alone, and a '
        're-install is idempotent under a different source ref', () {
      final f = rootFixture();
      const m = OverlayMaterializer();

      final first = m.materializeSync(
        resolution: f.resolution(),
        targetRoot: target.path,
        sourceRef: 'ref1',
      );
      final onDisk = File(
        p.join(target.path, kAgentsRootRelativePath),
      ).readAsStringSync();
      ...
      expect(first.written.single.relativePath, kAgentsRootRelativePath);
      expect(onDisk, startsWith(kGeneratedBlockBegin));
      expect(onDisk, endsWith('$kGeneratedBlockEnd\n'));
      expect(
        const MarkedBlockProvenance().bodyOf(onDisk),
        fixtureAgentsInstructionsBody(),
      );
      expect(second.written, isEmpty);
      expect(second.unchanged.single.relativePath, kAgentsRootRelativePath);
      expect(
        File(p.join(target.path, kAgentsRootRelativePath)).readAsStringSync(),
        onDisk,
        reason: 'a stale ref alone is not drift',
      );
    });
    ...
    test('bd block is preserved while the generated AGENTS block composes', () {
      ...
      final file = _write(target, [kAgentsRootRelativePath], existing);
      ...
      expect(report.updated.single.relativePath, kAgentsRootRelativePath);
      expect(report.blocked, isEmpty);
      expect(
        after.substring(0, existing.length),
        existing,
        reason: 'every pre-existing byte survives, in order and unedited',
      );
      expect(after, contains(_beadsBlock));
      expect(
        const MarkedBlockProvenance().bodyOf(after),
        fixtureAgentsInstructionsBody(),
      );
      expect(after, report.updated.single.contents);
    });

    test('only the OWNED block is replaced when the vended body drifts — the '
        'prose above it and below it is byte-for-byte untouched', () {
      const suffix = '\n## House rules\n\nkeep me exactly\n';
      final stale = stampProvenance(
        fixtureAgentsInstructionsBody('the OLD rule'),
        relativePath: kAgentsRootRelativePath,
        sourceRef: 'old',
        runner: 'space',
      );
      final file = _write(target, [
        kAgentsRootRelativePath,
      ], '$_repoPreamble$_beadsBlock$stale$suffix');

      const OverlayMaterializer().materializeSync(
        resolution: rootFixture('the NEW rule').resolution(),
        targetRoot: target.path,
        sourceRef: 'testref',
      );

      final after = file.readAsStringSync();
      expect(after, startsWith('$_repoPreamble$_beadsBlock'));
      expect(after, endsWith(suffix));
      expect(
        const MarkedBlockProvenance().bodyOf(after),
        fixtureAgentsInstructionsBody('the NEW rule'),
      );
      expect(after, isNot(contains('the OLD rule')));
    });

    test('an existing AGENTS file with neither marker is blocked '
        'byte-for-byte', () {
      const hand = '# power_station\n\nI wrote every line of this.\n';
      final file = _write(target, [kAgentsRootRelativePath], hand);

      final report = const OverlayMaterializer().materializeSync(
        resolution: rootFixture().resolution(),
        targetRoot: target.path,
        sourceRef: 'testref',
      );

      final blocked = report.blocked.single;
      expect(blocked.relativePath, kAgentsRootRelativePath);
      expect(blocked.symlink, isFalse);
      expect(blocked.reason, contains(kBeadsCodexBlockBeginPrefix));
      expect(
        blocked.checkClassification,
        OverlayCheckClassification.handEdited,
      );
      expect(file.readAsStringSync(), hand);
      expect(report.written, isEmpty);
      expect(report.updated, isEmpty);
    });

    test('a marker shape this tooling cannot CLAIM is blocked too — an '
        'unstamped imitation, a partial pair, a duplicated pair', () {
      ...
      final shapes = <String, String>{
        'unstamped':
            ...
            '${fixtureAgentsInstructionsBody()}'
            '$kGeneratedBlockEnd\n',
        'partial': '$_repoPreamble$kGeneratedBlockBegin\nno end marker\n',
        'duplicated': '$block$block',
      };

      for (final entry in shapes.entries) {
        final root = Directory(p.join(temp.path, 'shape-${entry.key}'));
        final file = _write(root, [kAgentsRootRelativePath], entry.value);
        ...
        expect(
          report.blocked.single.reason,
          contains('cannot claim'),
          reason: '${entry.key} is not a block this tooling wrote',
        );
        expect(file.readAsStringSync(), entry.value, reason: entry.key);
      }
    });

    test('a generated root AGENTS block becomes stale without sweeping', () {
      final f = fixture(assets: <GridAssetDefinition>[fixtureSkill('kept')]);
      final withdrawn =
          '$_repoPreamble$_beadsBlock'
          '${stampProvenance(fixtureAgentsInstructionsBody(), relativePath: kAgentsRootRelativePath, sourceRef: 'old', runner: 'space')}';
      final file = _write(target, [kAgentsRootRelativePath], withdrawn);

      final write = const OverlayMaterializer().materializeSync(
        resolution: f.resolution(),
        targetRoot: target.path,
        sourceRef: 'testref',
      );
      final check = const OverlayMaterializer().materializeSync(
        resolution: f.resolution(),
        targetRoot: target.path,
        sourceRef: 'testref',
        dryRun: true,
      ...
      expect(write.stale.single.reason, contains('the BLOCK is stale'));
      expect(
        scoped.stale,
        isEmpty,
        reason: 'the worktree leg never reaches a repository-root file',
      );
      expect(
        file.readAsStringSync(),
        withdrawn,
        reason: 'reported, never swept — the file around the block is theirs',
      );
    });

    test('an UNSTAMPED root file is not reported stale at all — this lib '
        'speaks only about blocks it generated', () {
      final f = fixture(assets: <GridAssetDefinition>[fixtureSkill('kept')]);
      _write(target, [kAgentsRootRelativePath], '$_repoPreamble$_beadsBlock');
    ...
    test('a root AGENTS symlink is blocked before it escapes the target', () {
      final outside = _write(
        Directory(p.join(temp.path, 'elsewhere')),
        [kAgentsRootRelativePath],
        'the REAL file, outside the target root',
      );
      Link(p.join(target.path, kAgentsRootRelativePath))
        ..parent.createSync(recursive: true)
        ..createSync(outside.path);
    ...
    test('a --check plans the composition without writing a byte', () {
      const existing = '$_repoPreamble$_beadsBlock';
      final file = _write(target, [kAgentsRootRelativePath], existing);
      ...
      expect(
        report.updated.single.checkClassification,
        OverlayCheckClassification.drifted,
      );
      expect(file.readAsStringSync(), existing);
    });

    test('the root file is NEVER materialized into a worktree — the scoped leg '
        'takes skill trees only', () {
      final f = rootFixture();
      ...
      expect(report.files, isEmpty);
      expect(
        File(p.join(target.path, kAgentsRootRelativePath)).existsSync(),
        isFalse,
      );
      expect(
        kWorktreeOverlaySubtrees,
        isNot(contains(kAgentsRootRelativePath)),
      );
      expect(kOverlayTargetHeads, isNot(contains(kAgentsRootRelativePath)));
    });
  });
}
